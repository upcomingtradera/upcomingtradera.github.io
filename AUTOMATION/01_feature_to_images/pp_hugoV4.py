...
def xxurlize(s: str) -> str:
    """Emulate Hugo's urlize function."""
    s = s.lower()
    # Handle special abbreviations and punctuations
    s = s.replace("vs.", "vs")  # Replace 'vs.' with 'vs'

    s = s.lower().replace(" ", "-")
    s = URLIZE_PATTERN.sub('', s)
    s = re.sub(r'[-]+', '-', s)
    s = s.strip('-')  # Remove trailing and leading hyphens
    return s
...
def urlize(s: str) -> str:
    ...
    # Step 2 and 3: Replace non-alphanumeric characters with a hyphen
    s = re.sub(r'[^a-z0-9]', '-', s)
...
def xxxprocess_frontmatter(content, config, current_dir):
    ...
    if not image_path:
        ...
        return
    # print(f"PROCESS_FRONTMATTER::[CURRENT_DIR]::[{current_dir}]")
    absolute_image_path = os.path.join(current_dir, image_path)
    
    config['frontmatter'] = frontmatter
    # print(f"PROCESS_FRONTMATTER::[FRONTMATTER]::[{frontmatter}]")
...
def process_frontmatter(content, config, current_dir):
    ...
    # Check if the frontmatter is empty and print content for debugging
    if not frontmatter:
        print(f"\t\tError: Empty frontmatter returned. Content was: {content} -- {current_dir}")
        return

    # Extracting the image path from the frontmatter
    image_path = frontmatter.get(config["input"]["key"])

    # Check if the image path is not found and print frontmatter for debugging
    if not image_path:
        print("Error: No image path found in frontmatter. Frontmatter content:\n", frontmatter)
        return

    # Processing the image
    absolute_image_path = os.path.join(current_dir, image_path)
    # print("Processing Image. Current Directory:", current_dir, "Image Path:", absolute_image_path)

    # Update config with frontmatter and continue processing
    config['frontmatter'] = frontmatter
    process_image(absolute_image_path, config)
            ...
            for size, postfix in zip(sizes, postfixes):
                ...
                for format in config["settings"]["formats"]:
                    filename = config["output"]["filename_template"].format(
                        frontmatter_title=urlize(title),
        ...
        for image_type in config["image_types"]:
            ...
            for size, postfix in zip(sizes, postfixes):
                ...
                for format in formats:
                    ...
                    # output_file = os.path.join(os_['dir'], frontmatter_title)
                    try:
...
def main(args):
    ...
    # Check if the configuration contains multiple jobs
    if 'jobs' in config:
...
def process_job(config):
    """Process a single job based on its configuration."""
    # Print the parsed configuration
    log_configuration(config)
